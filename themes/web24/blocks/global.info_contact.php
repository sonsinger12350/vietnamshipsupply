...
if (! nv_function_exists('nv_info_contact')) {
    function nv_info_contact_config($module, $data_block, $lang_block)
    {
        ...
        $html .= '</div>';
        $html .= '<div class="col-xs-8 hidden">';
        ...
        $html .= '</div>';
        $html .= '</div>';
        $html .= '<div id="config_company_maparea" class="hidden">';
        ...
        $html .= '<td>Link address:</td>';
        $html .= '<td><input type="text" class="form-control" name="config_company_link_address" value="' . $data_block['company_link_address'] . '"></td>';
        $html .= '</tr>';
        $html .= '<tr>';
        ...
        $html .= '</tr>';
        $html .= '<tr class="hide">';
        $html .= '<td colspan="2"><script type="text/javascript">$.getScript("' . NV_BASE_SITEURL . 'themes/' . $selectthemes . '/js/block.global.info_contact.js");</script></td>';
        $html .= '</tr>';

        return $html;
    }

    function nv_info_contact_submit()
    {
        global $nv_Request;
        ...
        $return['config']['company_link_address'] = $nv_Request->get_title('config_company_link_address', 'post');
     ...
     * @return
     */
    function nv_info_contact($block_config)
    {
        global $global_config, $lang_global;

        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/blocks/global.info_contact.tpl')) {
            $block_theme = $global_config['module_theme'];
        } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/blocks/global.info_contact.tpl')) {
        ...
        $xtpl = new XTemplate('global.info_contact.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks');
...
if (defined('NV_SYSTEM')) {
    $content = nv_info_contact($block_config);
}
