...
if (! nv_function_exists('nv_company_info')) {
    function nv_company_info_config($module, $data_block, $lang_block)
    {
        ...
        $html .= '<tr>';
        $html .= '<td>' . $lang_global['company_workshop'] . '</td>';
        $html .= '<td>';
        $html .= '<input type="text" class="form-control" name="config_company_workshop" id="config_company_workshop" value="' . $data_block['company_workshop'] . '">';
        $html .= '</td>';
        $html .= '</tr>';
        ...
        $html .= '<tr>';
        $html .= '<td>' . $lang_global['company_mobile'] . '</td>';
        $html .= '<td>
                    <div class="form-group"><input type="text" class="form-control" name="config_company_mobile" value="' . $data_block['company_mobile'] . '"></div>
                    <button class="btn btn-default btn-xs" onclick="modalShow(\'' . $lang_global['phone_note_title'] . '\',\'' . $lang_global['phone_note_content'] . '\');return!1;">' . $lang_global['phone_note_title'] . '</button>
                  </td>';
        $html .= '</tr>';
        ...
        $html .= '<tr>';
        $html .= '<td>' . $lang_global['company_trade_net'] . '</td>';
        $html .= '<td><input type="text" class="form-control" name="config_company_trade_net" value="' . $data_block['company_trade_net'] . '"><span></td>';
        $html .= '</tr>';
        $html .= '<tr>';
        $html .= '<td>' . $lang_global['company_mespas'] . '</td>';
        $html .= '<td><input type="text" class="form-control" name="config_company_mespas" value="' . $data_block['company_mespas'] . '"></td>';
        $html .= '</tr>';
    ...
    function nv_company_info_submit()
    {
        ...
        $return['config']['company_workshop'] = $nv_Request->get_title('config_company_workshop', 'post');
        ...
        $return['config']['company_mobile'] = $nv_Request->get_title('config_company_mobile', 'post');
        ...
        $return['config']['company_trade_net'] = $nv_Request->get_title('config_company_trade_net', 'post');
        $return['config']['company_mespas'] = $nv_Request->get_title('config_company_mespas', 'post');
    ...
    function nv_company_info($block_config)
    {
        ...
        if (! empty($block_config['company_workshop'])) {
            $xtpl->parse('main.company_workshop');
        }
        ...
        if (! empty($block_config['company_mobile'])) {
            $nums = array_map('trim', explode('|', nv_unhtmlspecialchars($block_config['company_mobile'])));
            foreach ($nums as $k => $num) {
                unset($m);
                if (preg_match("/^(.*)\s*\[([0-9\+\.\,\;\*\#]+)\]$/", $num, $m)) {
                    $xtpl->assign('MOBILE', array( 'number' => nv_htmlspecialchars($m[1]), 'href' => $m[2] ));
                    $xtpl->parse('main.company_mobile.item.href');
                    $xtpl->parse('main.company_mobile.item.href2');
                } else {
                    $num = preg_replace("/\[[^\]]*\]/", "", $num);
                    $xtpl->assign('MOBILE', array( 'number' => nv_htmlspecialchars($num) ));
                }
                if ($k) {
                    $xtpl->parse('main.company_mobile.item.comma');
                }
                $xtpl->parse('main.company_mobile.item');
            }

            $xtpl->parse('main.company_mobile');
        }
        ...
        if (! empty($block_config['company_trade_net'])) {
            $xtpl->parse('main.company_trade_net');
        }
        if (! empty($block_config['company_mespas'])) {
            $xtpl->parse('main.company_mespas');
        }
