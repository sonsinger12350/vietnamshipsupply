...
function nv_site_theme($contents, $full = true)
{
    ...
    if ($global_config['current_theme_type'] == 'r') {
		...
		$html_links[] = array(
			'rel' => 'StyleSheet',
			'href' => NV_BASE_SITEURL . 'themes/' . $global_config['module_theme'] . '/css/uikit/uikit.min.css'
		);
        // $html_links[] = array(
            // 'rel' => 'StyleSheet',
	...
	$html_links_preload = $html_links;
    foreach ($html_links_preload as $links_pre) {
		if($links_pre['rel'] == 'canonical'){
			continue;
		}
        $links_pre['rel'] = 'preload';
        $links_pre['as'] = 'style';
        foreach ($links_pre as $key => $value) {
            $xtpl->assign('LINKS', array(
                'key' => $key,
                'value' => $value
            ));
            if (!empty($value)) {
                $xtpl->parse('main.links_pre.attr.val');
            }
            $xtpl->parse('main.links_pre.attr');
        }
        $xtpl->parse('main.links_pre');
    }
