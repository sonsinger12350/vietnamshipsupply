			...
			<div class="col-xs-24 col-md-4">
				<div class="form-group">
					<select class="form-control" name="searchfor">
						<option value="all">---{LANG.searchfor}---</option>
						<!-- BEGIN: loopsearch -->
						<option {sl_search} value="{key_search}">{val_search}</option>
						<!-- END: loopsearch -->
					</select>
				</div>
			</div>
	...
	<table class="table table-striped table-bordered table-hover">
		...
		<tbody>
			...
			<tr>
				...
				<td><a href="javascript:void(0);" onclick="nv_view_transaction('{CONTENT.id}');">{CONTENT.code}</a></td>
				<td><a onclick="nv_view_transaction_all({CONTENT.customer_id});" href="javascript:void(0);">{CONTENT.customer_name}</a></td>
				<td>{CONTENT.userid}</td>
				<td align="right">{CONTENT.status}{CONTENT.money_total}</td>
				<td class="text-center">{CONTENT.created_time}</td>
				<td class="text-center"><em class="fa fa-search">&nbsp;</em><a href="javascript:void(0);" onclick="nv_view_transaction('{CONTENT.id}');">{LANG.viewdetail}</a></td>
			</tr>
			<!-- END: loop -->
		</tbody>
	</table>
