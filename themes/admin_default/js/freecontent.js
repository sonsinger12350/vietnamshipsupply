...
$(document).ready(function(){	
		...
		if( isEditor ){
		    ...
		    if( ! instance ){
				CKEDITOR.replace(cfg.ctEditor, {
				    ...
				    toolbar: [{ name: 'Tools', items: [ 'Undo', 'Redo', '-', 'Bold', 'Italic', 'Underline', '-', 'RemoveFormat', '-', 'JustifyLeft', 'JustifyCenter', 'JustifyRight', 'JustifyBlock', '-',  'NumberedList', 'BulletedList', '-', 'Link', 'TextColor', '-', 'Source', '-', 'Maximize' ] }],
	...
	$(cfg.ctModal + ' form').submit(function(e){
		...
		var data = {
			...
			subdes: $this.find('[name="subdes"]').val(),
			bid: $this.find('[name="bid"]').val(),
			title: $this.find('[name="title"]').val(),
			description: $this.find('[name="description"]').val(),
