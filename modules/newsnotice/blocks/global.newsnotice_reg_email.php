...
if (! nv_function_exists('nv_function_reg_email')) {
    ...
    {
        global $global_config, $module_info, $lang_module, $site_mods;
        
        $module = $block_config['module'];
        $mod = $site_mods[$module]['module_file'];
        ...
        if (file_exists(NV_ROOTDIR . "/modules/" . $mod . "/language/" . NV_LANG_INTERFACE . ".php")) {
            require_once NV_ROOTDIR . "/modules/" . $mod . "/language/" . NV_LANG_INTERFACE . ".php";
        }
        $lang_module = $lang_module + $lang_temp;
        unset($lang_temp);
        
        $xtpl = new XTemplate('global.newsnotice_reg_email.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $mod);
