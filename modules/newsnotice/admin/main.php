...
$sql = "SELECT catid, alias FROM " . NV_PREFIXLANG . "_".$array_config['mod_data']."_cat ORDER BY weight ASC";
try {
...
while (list ($catid_i, $alias_i) = $result_cat->fetch(3)) {
    $global_array_cat_news[$catid_i] = NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=".$array_config['mod_name']."&amp;" . NV_OP_VARIABLE . "=" . $alias_i;
}
...
while ($row = $result->fetch()) {
    ...
    foreach ($listid as $id) {
        $post = $db->query("SELECT id, title, alias, catid FROM " . NV_PREFIXLANG . "_".$array_config['mod_data']."_rows WHERE status = 1 AND id = " . $id)->fetch();
        if ($post) {
            $i ++;
