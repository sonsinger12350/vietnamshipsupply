...
elseif( ACTION_METHOD == 'add' || ACTION_METHOD == 'edit' )
{
	...
	if( $nv_Request->get_int( 'save', 'post' ) == 1 )
	{
		...
		$data['description2'] = $nv_Request->get_textarea('description2', '', 'br', 1);
		$data['description3'] = $nv_Request->get_textarea('description3', '', 'br', 1);
		$data['description4'] = $nv_Request->get_textarea('description4', '', 'br', 1);
		$data['description5'] = $nv_Request->get_textarea('description5', '', 'br', 1);
		...
		if( empty( $error ) )
		{
			...
			if( $data['photo_id'] == 0 )
			{
				try
				{
					...
					$stmt->bindParam( ':description2', $data['description2'], PDO::PARAM_STR, strlen($data['description2']));
					$stmt->bindParam( ':description3', $data['description3'], PDO::PARAM_STR, strlen($data['description3']));
					$stmt->bindParam( ':description4', $data['description4'], PDO::PARAM_STR, strlen($data['description4']));
					$stmt->bindParam( ':description5', $data['description5'], PDO::PARAM_STR, strlen($data['description5']));
		...
		if( empty( $error ) )
		{
			...
			Header( 'Location: ' . NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op . '&group_id=' . $data['group_id'] );
			die();
		}
