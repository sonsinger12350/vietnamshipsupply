...
if (!nv_function_exists('nv_news_category')) {
     ...
     * @return
     */
    function nv_news_category($block_config, $module)
    {
        global $module_array_cat, $lang_module, $global_config, $site_mods, $lang_global, $nv_Cache;
		...
		if(!defined('NV_IS_MODADMIN')){
			$cache_file = NV_LANG_DATA . '_block_category_' . $block_config['bid'] . '_' . NV_CACHE_PREFIX . '.cache';
		}
        ...
        if (($cache = $nv_Cache->getItem($module, $cache_file)) != false) {
			...
			if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/business/block_category.tpl')) {
				$block_theme = $global_config['module_theme'];
			} else {
				$block_theme = 'default';
			}

			$xtpl = new XTemplate('block_category.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/business');
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        if ($module == $module_name) {
            ...
            $sql = "SELECT catid, parentid, title, alias, viewcat, subcatid, numlinks, description, image, keywords, groups_view, status FROM " . NV_PREFIXLANG . "_" . $site_mods[$module]['module_data'] . "_cat ORDER BY sort ASC";
        ...
        $content = nv_news_category($block_config, $module);
    }
}
