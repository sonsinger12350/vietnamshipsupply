...
if ($all_page) {
    ...
    while ($row = $result->fetch()) {
        // var_dump($row);die();
        if (!empty($row['img'])) {
            $imageinfo = nv_ImageInfo(NV_ROOTDIR . '/' . $row['img'], 250, true, NV_ROOTDIR . '/' . NV_FILES_DIR . '/' . $module_name);

            $row['img'] = $imageinfo['src'];
			$row['img_upload'] = $imageinfo['orig_src'];
        }
        elseif(!empty($row['externalpath'])){
            if(preg_match("/^(http(s)?\:)?\/\/([w]{3})?\.youtube[^\/]+\/watch\?v\=([^\&]+)\&?(.*?)$/is", $row['externalpath'], $m)){
                $row['img'] = 'https://i3.ytimg.com/vi/'.$m[4].'/hqdefault.jpg';
                $code = $m[4];
            } elseif (preg_match("/(http(s)?\:)?\/\/youtu?\.be[^\/]?\/([^\&]+)$/isU", $row['externalpath'], $m)) {
                $row['img'] = 'https://i3.ytimg.com/vi/'.$m[3].'/hqdefault.jpg';
                $code = $m[3];
            }
        } 
        else {
