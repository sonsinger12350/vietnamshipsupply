...
$cat_data = $nv_Request->get_title('cat_data', 'post');
if(!empty($cat_data)){
    $sql_news_cat = 'SELECT * FROM ' . NV_PREFIXLANG .'_'. $cat_data . '_cat ORDER BY sort ASC';
    $result_cat = $db->query($sql_news_cat)->fetchAll();
    $html = '';
    $html .= '<option value="0" >Không chọn</option>';
    foreach ($result_cat as $result_cat_i) {
        $xtitle_i = '';
        if($result_cat_i['lev'] > 0){
            for ($i = 1; $i <= $result_cat_i['lev']; ++$i) {
                $xtitle_i .= '---&nbsp;';
            }
        }
        $html .= '<option value="'.$result_cat_i['catid'].'" >'.$xtitle_i .$result_cat_i['title'].'</option>';
    }
    print($html);die();
}
...
if ($nv_Request->isset_request('add', 'get')) {
    ...
    if ($nv_Request->isset_request('submit', 'post')) {
        ...
        $array['in_news_module'] = $nv_Request->get_title('in_news_module', 'post,get', '');
        $array['in_news_cat'] = $nv_Request->get_int('in_news_cat', 'post,get', 0);
    ...
    $xtpl->assign('URL_AJAX', NV_BASE_ADMINURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=" . $op);
    foreach ($site_mods as $mod) {
        if($mod['module_file'] == 'news'){
            $selected = ($array['in_news_module'] == $mod['module_data']) ? 'selected="selected"' : '';
...
if ($nv_Request->isset_request('edit', 'get')) {
    ...
    if ($nv_Request->isset_request('submit', 'post')) {
        ...
        $array['in_news_module'] = $nv_Request->get_title('in_news_module', 'post', '', 1);
        $array['in_news_cat'] = $nv_Request->get_int('in_news_cat', 'post', '', 1);
        ...
        if (!$is_error) {
            ...
            in_news_module=" . $db->quote($array['in_news_module']) . ",
            in_news_cat=" . $array['in_news_cat'] . ",
        ...
        $array['in_news_module'] = $row['in_news_module'];
        $array['in_news_cat'] = $row['in_news_cat'];
    ...
    if ($array['in_news_module'] != '0' and $array['in_news_cat'] > 0) {        
        $sql_news_cat = 'SELECT * FROM ' . NV_PREFIXLANG .'_'. $array['in_news_module'] . '_cat ORDER BY sort ASC';
        $result_cat = $db->query($sql_news_cat)->fetchAll();
        foreach ($result_cat as $result_cat_i) {
            $xtitle_i = '';
            if($result_cat_i['lev'] > 0){
            ...
            $selected = ($array['in_news_cat'] == $result_cat_i['catid']) ? 'selected="selected"' : '';
            $xtpl->assign('CAT_SELECTED', $selected);
            $xtpl->assign('CAT_DATA', $result_cat_i['catid']);
            $xtpl->assign('CAT_NAME', $xtitle_i.$result_cat_i['title']);
            $xtpl->parse('main.list_news_cat');
        }
    }
