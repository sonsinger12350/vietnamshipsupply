...
{
	$where = ' AND (' . nv_like_logic('description', $dbkeyword, $logic) . ' OR ' . nv_like_logic('bodytext', $dbkeyword, $logic) . ')';
}
...
if ($num_items) {
    ...
    $db_slave->select('id,title, alias, image, description, bodytext')
        ->limit($limit)
        ->offset(($page - 1) * $limit);
    $result = $db_slave->query($db_slave->sql());
    while (list($id, $tilterow, $alias, $image, $description, $content) = $result->fetch(3)) {
