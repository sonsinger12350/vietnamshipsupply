...
if ($module_config[$m_values['module_name']]['elas_use'] == 1) {
    ...
    if ($num_items) {
        ...
        $db_slave->select('r.id, r.title, r.alias, r.homeimgfile, r.catid, r.hometext, c.bodyhtml')
            ->order('publtime DESC')
            ->limit($limit)
            ->offset(($page - 1) * $limit);
        $result = $db_slave->query($db_slave->sql());
        while (list ($id, $tilterow, $alias, $image, $catid, $hometext, $bodytext) = $result->fetch(3)) {
            $content = strip_tags($hometext, 'br') . strip_tags($bodytext);
