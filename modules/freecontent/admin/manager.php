...
if ($nv_Request->isset_request('getinfo', 'post')) {
    ...
    if ($id) {
        $sth = $db->prepare('SELECT title, subdes, description, link, target, image, icon, start_time, end_time, status FROM ' . NV_PREFIXLANG . '_' . $module_data . '_rows WHERE id=:id');
        ...
        if (! empty($array)) {
            ...
            $array['icon'] = html_entity_decode($array['icon']);
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    $data['subdes'] = nv_substr($nv_Request->get_title('subdes', 'post', ''), 0, 255);
    ...
    $data['icon'] = htmlentities($nv_Request->get_editor('icon', 'post'));
    ...
    if (empty($error)) {
        if ($data['id']) {
            $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_rows SET
				title = :title, subdes = :subdes, description = :description, link = :link, target = :target, image = :image, icon = :icon, start_time = :start_time, end_time = :end_time, status = :status
			WHERE id = ' . $data['id'];
        } else {
            $sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_rows (bid, title, subdes, description, link, target, image, icon, start_time, end_time, status) VALUES (
				' . $bid . ', :title, :subdes, :description, :link, :target, :image, :icon, :start_time, :end_time, :status
			)';
        }
        ...
        try {
            ...
            $sth->bindParam(':subdes', $data['subdes'], PDO::PARAM_STR);
