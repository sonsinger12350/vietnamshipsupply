...
if (!nv_function_exists('nv_group_hotdeal')) {
     ...
     *
     */
    function nv_block_config_group_hotdeal_blocks($module, $data_block, $lang_block)
    {
     ...
     *
     */
    function nv_block_config_group_hotdeal_blocks_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
     ...
     *
     */
    function nv_group_hotdeal($block_config)
    {
        ...
        $xtpl = new XTemplate('block.group_hotdeal.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $mod_file);
        $xtpl->assign('LANG', $lang_module);
        $xtpl->assign('WIDTH', $pro_config['homewidth']);

		$db->sqlreset()
			->select('t1.id, t1.listcatid, t1.' . NV_LANG_DATA . '_title AS title, t1.' . NV_LANG_DATA . '_alias AS alias, t1.addtime, t1.homeimgfile, t1.homeimgthumb, t1.product_price, t1.money_unit, t1.discount_id, t1.showprice, t1.product_number, t2.end_time')
			...
			// ->where('t2.begin_time <= ' . NV_CURRENTTIME . ' AND t2.end_time > '. NV_CURRENTTIME .' AND t1.discount_id > 0 AND t1.product_number > 0 AND t1.status =1')
			->where(NV_CURRENTTIME . ' BETWEEN t2.begin_time AND t2.end_time AND t1.discount_id > 0 AND t1.product_number > 0 AND t1.status =1')
        ...
        foreach ($list as $row) {
            ...
            $xtpl->assign('end_time', $row['end_time']);
            $xtpl->assign('remain', $row['product_number']);
...
if (defined('NV_SYSTEM')) {
    $content = nv_group_hotdeal($block_config);
}
