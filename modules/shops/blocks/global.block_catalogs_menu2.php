...
if (!function_exists('nv_pro_catalogs_menu2')) {

    /**
     * nv_block_config_product_catalogs_menu2_blocks()
     *
     * @param mixed $module
     ...
     * @return
     */
    function nv_block_config_product_catalogs_menu2_blocks($module, $data_block, $lang_block)
    {
    ...
    /**
     * nv_block_config_product_catalogs_menu2_blocks_submit()
     *
     * @param mixed $module
     * @param mixed $lang_block
     * @return
     */
    function nv_block_config_product_catalogs_menu2_blocks_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    /**
     * nv_pro_catalogs_menu2()
     *
     * @param mixed $block_config
     * @return
     */
    function nv_pro_catalogs_menu2($block_config)
    {
        ...
        $block_tpl_name = "block.catalogs_menu2.tpl";
        ...
        foreach ($array_cat_shops as $cat) {
            if ($cat['parentid'] == 0) {
				...
				if (!empty($cat['subcatid'])) {
					$sub_cat = html_viewsub_menu2_cat($cat['subcatid'], $block_config, $module, $mod_file);
     ...
     * @return
     */
    function html_viewsub_menu2_cat($list_sub, $block_config, $module, $mod_file)
    {
        ...
        if (empty($list_sub)) {
			...
			$xtpl = new XTemplate("block.catalogs_menu2.tpl", NV_ROOTDIR . "/themes/" . $global_config['module_theme'] . "/modules/" . $mod_file);
            ...
            foreach ($list as $catid) {
				...
				if (!empty($array_cat_shops[$catid]['subcatid'])) {
					$cat_sub = html_viewsubsub_menu2_cat($array_cat_shops[$catid]['subcatid'], $block_config, $module, $mod_file);
					$xtpl->assign('CAT_SUBSUBCAT', $cat_sub);
					$xtpl->parse('cat_submenu.loop.subcat');
				}
	...
	function html_viewsubsub_menu2_cat($list_sub, $block_config, $module, $mod_file)
    {
        global $array_cat_shops, $global_config;
        $cut_num = $block_config['cut_num'];
        if (empty($list_sub)) {
            ...
            foreach ($list as $catid) {
				...
				if (!empty($array_cat_shops[$catid]['subcatid'])) {
					$cat_sub = html_viewsubsub_menu2_cat($array_cat_shops[$catid]['subcatid'], $block_config, $module, $mod_file);
					$xtpl->assign('CAT_SUBSUBCAT', $cat_sub);
					$xtpl->parse('cat_subsubmenu.loop.subcat');
				}
				$xtpl->assign('CAT_SUBCAT', $array_cat_shops[$catid]);
				$xtpl->parse('cat_subsubmenu.loop');
            }
			
			$xtpl->parse('cat_subsubmenu');
			return $xtpl->text('cat_subsubmenu');
        }
    }
}

if (defined('NV_SYSTEM')) {
    $content = nv_pro_catalogs_menu2($block_config);
}
