 ...
 * @return
 */
function nv_template_detail($data_content, $data_unit, $data_others, $array_other_view, $content_comment, $compare_id, $popup, $idtemplate, $array_tags)
{
    ...
    if (!empty($data_content)) {
		...
		if($data_content['product_number'] > 0){
			$xtpl->assign('availability', 'InStock'); 
		}
		else{
			$xtpl->assign('availability', 'SoldOut');
		}
        ...
        if (!$popup) {
            ...
            if (!empty($array_tags)) {
                $t = sizeof($array_tags) - 1;
                foreach ($array_tags as $i => $value) {
                    $xtpl->assign('TAGS', $value['title']);
                    ...
                    $xtpl->parse('main.product_detail.tags.loop');
                }
                $xtpl->parse('main.product_detail.tags');
            }

            if (!empty($data_others)) {
                $html = call_user_func('nv_template_viewgrid', $data_others, '', 0);
...
function cart_product($data_content, $coupons_code, $order_info, $array_error_number)
{
    ...
    $xtpl->assign('LINK_CART_UPDATE', nv_url_rewrite(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=cart', true));
...
{
    global $module_info, $lang_module, $lang_global, $module_config, $module_data, $module_file, $module_name, $pro_config, $money_config, $global_array_group, $shipping_data, $db, $db_config;
    ...
    // LẤY TỈNH THÀNH RA
    $list_tinhthanh = $db->query('SELECT * FROM nv4_location_province WHERE status = 1 ORDER BY weight DESC')->fetchAll();    
    foreach($list_tinhthanh as $tinhthanh){
        $xtpl->assign('l', $tinhthanh);
        $xtpl->parse('main.tinh');
    }
    if($data_order['user_id'] > 0){
        $result_point = $db->query("SELECT * FROM ".$db_config['prefix']."_".$module_data."_point WHERE userid = " . $data_order['user_id'])->fetch();
    }

    if (!empty($result_point) && $result_point['point_total'] > 0) {
        $result_convernt = $db->query("SELECT * FROM " . NV_CONFIG_GLOBALTABLE . " WHERE lang = '" . NV_LANG_DATA . "' AND module = ".$db->quote($module_name)." AND config_name = 'point_active' OR config_name = 'point_conversion'")->fetchAll();
        foreach ($result_convernt as $result_convernt_i) {
            if($result_convernt_i['config_name'] == "point_conversion"){
                $result_convernt_conversion = $result_convernt_i['config_value'];
            }
            else{
                $result_convernt_active = $result_convernt_i['config_value'];
            }
        }
        if($result_convernt_conversion > 0 && $result_convernt_active = 1){
            $point_money = $result_point['point_total'] * $result_convernt_conversion;
            $xtpl->assign('ratio_money', $result_convernt_conversion);
            $xtpl->assign('point_num', $result_point['point_total']);
            $xtpl->assign('point_money', nv_number_format($point_money,0));
...
{
    global $module_info, $lang_module, $module_data, $module_file, $global_config, $module_name, $pro_config, $money_config, $global_array_group, $client_info, $array_location, $array_shops, $db;
    ...
    // LẤY TỈNH THÀNH RA
    $tinhthanh = $db->query('SELECT type,title FROM nv4_location_province WHERE status = 1 AND provinceid = '.$data_content['tinhthanh'])->fetch();
    // LẤY QUẬN HUYỆN RA
    $quanhuyen = $db->query('SELECT type,title FROM nv4_location_district WHERE status = 1 AND districtid = '.$data_content['quanhuyen'])->fetch();
    // LẤY XÃ PHƯỜNG RA
    $xaphuong = $db->query('SELECT type,title FROM nv4_location_ward WHERE status = 1 AND wardid = '.$data_content['xaphuong'])->fetch();
    
    if (!empty($tinhthanh)) {
        $xtpl->assign('xaphuong', $xaphuong['type'] . ": " .$xaphuong['title'] . " - ");
    }
    if (!empty($quanhuyen)) {
        $xtpl->assign('quanhuyen', $quanhuyen['type'] . ": " .$quanhuyen['title'] . " - ");
    }
    if (!empty($tinhthanh)) {
        $xtpl->assign('tinhthanh', $tinhthanh['type'] . ": " .$tinhthanh['title']);
    }
...
/**
 * nv_template_viewdeal
 *
 * @param mixed $array_data
 * @return
 */
function nv_template_viewdeal($array_data, $page = '')
{
    ...
    $xtpl = new XTemplate('hotdeal.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
    ...
    if (!empty($array_data)) {
        foreach ($array_data as $data_row) {
			...
			if($data_row['begin_time'] < NV_CURRENTTIME && NV_CURRENTTIME < $data_row['end_time']){ // Đang diễn ra
				if($pro_config['review_active']){
					if($data_row['average']){
						for ($i=1; $i <= $data_row['average']; $i++){
							$xtpl->parse('main.new.rating.star');
						}
					}
					$xtpl->parse('main.new.rating');
				}
				...
				if ($pro_config['active_price'] == '1') {
					if ($data_row['showprice'] == '1' && !empty($data_row['product_price'])) {
						...
						if ($data_row['discount_id'] and $price['discount_percent'] > 0) {
							$xtpl->parse('main.new.price.discounts');
							$xtpl->parse('main.new.price.discounts.standard');
						} else {
							$xtpl->parse('main.new.price.no_discounts');
						}
						$xtpl->parse('main.new.price');
					} else {
						$xtpl->parse('main.new.contact');
					}
				}
				...
				if ($pro_config['point_active'] and $global_array_shops_cat[$data_row['listcatid']]['cat_allow_point'] and !empty($global_array_shops_cat[$data_row['listcatid']]['cat_number_point'])) {
					...
					$xtpl->parse('main.new.point');
				}
				$xtpl->parse('main.new');
			}
			elseif($data_row['begin_time'] > NV_CURRENTTIME && $data_row['begin_time'] < $data_row['end_time']){ // Sắp diễn ra
				if($pro_config['review_active']){
					if($data_row['average']){
						for ($i=1; $i <= $data_row['average']; $i++){
							$xtpl->parse('main.coming.rating.star');
						}
					}
					$xtpl->parse('main.coming.rating');
				}
				...
				if ($pro_config['active_price'] == '1') {
					if ($data_row['showprice'] == '1' && !empty($data_row['product_price'])) {
						$xtpl->assign('PRICE', $price);
						if ($data_row['discount_id'] and $price['discount_percent'] > 0) {
							$xtpl->parse('main.coming.price.discounts');
							$xtpl->parse('main.coming.price.discounts.standard');
						} else {
							$xtpl->parse('main.coming.price.no_discounts');
						}
						$xtpl->parse('main.coming.price');
					} else {
						$xtpl->parse('main.coming.contact');
					}
				}
				...
				if ($pro_config['active_order'] == '1' and $pro_config['active_order_non_detail'] == '1') {
					if ($data_row['showprice'] == '1' && !empty($data_row['product_price'])) {
						if ($data_row['product_number'] > 0) {
							...
							$xtpl->parse('main.coming.order');
						} else {
							$xtpl->parse('main.coming.product_empty');
						}
					}
				}

				if (!empty($pro_config['show_product_code']) and !empty($data_row['product_code'])) {
					$xtpl->parse('main.coming.product_code');
				}

				if (defined('NV_IS_MODADMIN')) {
					$xtpl->assign('ADMINLINK', nv_link_edit_page($data_row['id']) . '&nbsp;&nbsp;' . nv_link_delete_page($data_row['id']));
					$xtpl->parse('main.coming.adminlink');
				}
	 
				if ($data_row['discount_id'] and $price['discount_percent'] > 0 and $data_row['showprice']) {
					$xtpl->parse('main.coming.discounts');
				}
				...
				if ($pro_config['point_active'] and $global_array_shops_cat[$data_row['listcatid']]['cat_allow_point'] and !empty($global_array_shops_cat[$data_row['listcatid']]['cat_number_point'])) {
					$xtpl->assign('point', $global_array_shops_cat[$data_row['listcatid']]['cat_number_point']);
					$xtpl->assign('point_note', sprintf($lang_module['point_product_note'], $global_array_shops_cat[$data_row['listcatid']]['cat_number_point']));
					$xtpl->parse('main.coming.point');
				}
				$xtpl->parse('main.coming');
			}
        }
    }
 ...
 * @return
 */
function nv_template_viewgrid($array_data, $page = '', $slide = 0)
{
    global $module_info, $lang_module, $lang_global, $module_name, $module_data, $module_file, $module_upload, $pro_config, $op, $compareid, $global_array_shops_cat;
    ...
    if($slide == 0)
        ...
        $xtpl = new XTemplate('viewgird_slide.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
