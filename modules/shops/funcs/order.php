...
if ($post_order == 1) {
    ...
    $data_order['tinhthanh'] = $nv_Request->get_title('tinhthanh', 'post', '');
    $data_order['quanhuyen'] = $nv_Request->get_title('quanhuyen', 'post', '');
    $data_order['xaphuong'] = $nv_Request->get_title('xaphuong', 'post', '');
    $data_order['httt'] = $nv_Request->get_title('httt', 'post', '');
   ...
   // $total += $total_weight_price;
    ...
    $data_order['usepoint'] = $nv_Request->get_int('usepoint', 'post', 0);
    $usedpoint = false;
    if ($data_order['usepoint'] == 1) {
        $result_check = $db->query("SELECT * FROM " . NV_CONFIG_GLOBALTABLE . " WHERE lang = '" . NV_LANG_DATA . "' AND module = ".$db->quote($module_name)." AND config_name = 'point_active' OR config_name = 'point_conversion'")->fetchAll(); // Check kích hoạt điểm
        foreach ($result_check as $result_check_i) {
            if($result_check_i['config_name'] == "point_conversion"){
                $result_check_conversion = $result_check_i['config_value'];
            }
            else{
                $result_check_active = $result_check_i['config_value'];
            }
        }
        if($result_check_active == 1){
            $result_point = $db->query("SELECT * FROM ".$db_config['prefix']."_".$module_data."_point WHERE userid = " . $data_order['user_id'])->fetch();
            if($result_point['point_total'] > 0){
                for ($point_using = $result_point['point_total']; $point_using >= 0 ; $point_using--) {
                    $point_money = $point_using * $result_check_conversion;
                    if($data_order['order_total'] >= $point_money){
                        $data_order['order_total'] -= $point_money;
                        $result_point['point_total'] -= $point_using;
                        $usedpoint = true;
                        break;
                    }
                    elseif ($data_order['order_total'] < $point_money) {
                        continue;
                    }
                }
            }
    ...
    if (empty($error) and $i > 0) {
        if (!empty($order_info)) {
			...
			order_phone = :order_phone, order_address = :order_address, order_note = :order_note, tinhthanh =:tinhthanh, quanhuyen =:quanhuyen, xaphuong =:xaphuong, httt =:httt, order_total = ' . doubleval($data_order['order_total']) . ',
            ...
            $sth->bindParam(':tinhthanh', $data_order['tinhthanh'], PDO::PARAM_INT);
            $sth->bindParam(':quanhuyen', $data_order['quanhuyen'], PDO::PARAM_INT);
            $sth->bindParam(':xaphuong', $data_order['xaphuong'], PDO::PARAM_INT);
            $sth->bindParam(':httt', $data_order['httt'], PDO::PARAM_STR);
            ...
            $sql = "INSERT INTO " . $db_config['prefix'] . "_" . $module_data . "_orders (
				lang, order_code, order_name, order_email, order_phone, order_address, order_note, tinhthanh, quanhuyen, xaphuong, httt, user_id, admin_id, shop_id, who_is, unit_total, order_total, point_using, point_money, order_time, postip, order_view,
				transaction_status, transaction_id, transaction_count
			) VALUES (
				'" . NV_LANG_DATA . "', :order_code, :order_name, :order_email, :order_phone, :order_address, :order_note, " . intval($data_order['tinhthanh']) . ", " . intval($data_order['quanhuyen']) . ", " . intval($data_order['xaphuong']) . ", :httt,
				" . intval($data_order['user_id']) . ", " . intval($data_order['admin_id']) . ", " . intval($data_order['shop_id']) . ",
				" . intval($data_order['who_is']) . ", :unit_total, " . doubleval($data_order['order_total']) . ",".intval($point_using).", :point_money, 
        ...
        if ($order_id > 0) {
            ...
            if (empty($order_info)) {
                ...
                if($usedpoint == true){ //Phần bổ sung
                    $db->query("UPDATE ".$db_config['prefix']."_".$module_data."_point SET point_total = ".$result_point['point_total']." WHERE userid = " . $data_order['user_id']);
                }
            ...
            // LẤY TỈNH THÀNH RA
            if (!empty($data_order['tinhthanh'])) {
                $laytinhthanh = $db->query('SELECT type,title FROM nv4_location_province WHERE status = 1 AND provinceid ='.$data_order['tinhthanh'])->fetch();
            }            
            // LẤY QUẬN HUYỆN RA
            if (!empty($data_order['quanhuyen'])) {
                $layquanhuyen = $db->query('SELECT type,title FROM nv4_location_district WHERE status = 1 AND districtid ='.$data_order['quanhuyen'])->fetch();
            }
            // LẤY XÃ PHƯỜNG RA
            if (!empty($data_order['xaphuong'])) {
                $layxaphuong = $db->query('SELECT type,title FROM nv4_location_ward WHERE status = 1 AND wardid ='.$data_order['xaphuong'])->fetch();
            }
            ...
            $replace_data = array(
                ...
                'tinhthanh' => $laytinhthanh['type'].' '.$laytinhthanh['title'],
                'quanhuyen' => $layquanhuyen['type'].' '.$layquanhuyen['title'],
                'xaphuong' => $layxaphuong['type'].' '.$layxaphuong['title'],
                'httt' => $data_order['httt'],
            ...
            $email_contents = call_user_func('email_new_order', $content, $data_order, $data_pro, $data_shipping);
            ...
            if (!empty($listmail_notify)) {
                $email_contents_to_admin = call_user_func('email_new_order', $content, $data_order, $data_pro, $data_shipping);
