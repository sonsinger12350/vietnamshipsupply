...
if (! empty($savecat)) {
	...
	$check_return = $check_alias->check_id_alias($data['bid'], $data['alias']);
    ...
    elseif (empty($data['title'])) {
        ...
        if ($data['bid'] == 0) {
            ...
            if ($stmt->rowCount()) {
                ...
                if ($insert_bid = $db->insert_id($sql, 'bid', $data_insert)) {
					
					$check_alias->add_block_shop_alias($insert_bid, $data['alias'], $module_name, 'blockcat');
            ...
            if ($stmt->rowCount()) {
                ...
                if ($stmt->execute()) {
					...
					$check_alias->add_block_shop_alias($data['bid'], $data['alias'], $module_name, 'blockcat');
