...
$rowcontent = array(
    ...
    'hitslm' => 0,
    'showprice' => 1,
    'keywords' => '', // Cái này là keyword cho thẻ meta mấy cái k có chữ _text là cho tag
    'com_id' => 0,
    'title' => '',
    'alias' => '',
    'hometext' => '',
...
if ($nv_Request->get_int('save', 'post') == 1) {
    ...
    if (empty($error)) {
        ...
        if ($error == '') {
			...
			// CẬP NHẬT TAG MỚI
			if ($rowcontent['tags'] != $rowcontent['tags_old'] || ($is_copy == 1 && $rowcontent['tags'] !='')) {
           ...
           // if ($rowcontent['keywords'] != $rowcontent['keywords_old'] || ($is_copy == 1 && $rowcontent['keywords'] !='')) {
            ...
            nv_redirect_location($_SESSION['url']);
        }
...
if (!empty($rowcontent['keywords'])) {
    $keywords_text_array = explode(',', $rowcontent['keywords']);
    foreach ($keywords_text_array as $keywords_text) {
        $xtpl->assign('KEYWORDS_TEXT', $keywords_text);
        $xtpl->parse('main.keywords_text');
    }
}

// Print tags
// if (!empty($rowcontent['tags'])) {
    // $keywords_array = explode(',', $rowcontent['tags']);
