...
// LẤY TỈNH THÀNH RA
$laytinhthanh = $db->query('SELECT type, title FROM nv4_location_province WHERE status = 1 AND provinceid ='.$data_content['tinhthanh'])->fetch();
// LẤY QUẬN HUYỆN RA
$layquanhuyen = $db->query('SELECT type, title FROM nv4_location_district WHERE status = 1 AND districtid ='.$data_content['quanhuyen'])->fetch();
// LẤY XÃ PHƯỜNG RA
$layxaphuong = $db->query('SELECT type, title FROM nv4_location_ward WHERE status = 1 AND wardid ='.$data_content['xaphuong'])->fetch();
// var_dump($laytinhthanh['title']);die();
$xtpl->assign('tinhthanh', $laytinhthanh['title']);
$xtpl->assign('quanhuyen', $layquanhuyen['title']);
$xtpl->assign('xaphuong', $layxaphuong['title']);
