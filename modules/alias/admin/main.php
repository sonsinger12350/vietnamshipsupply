...
{
	// Dùng để xóa các alias (thuộc module nào đó) đã không còn trong hệ thống
...
{
	// Dùng để xóa, cập nhật, thêm các alias (module đã chọn)
	...
	if(!empty($mod_file) and !empty($mod_data) and !empty($mod_name))
	{
		...
		if($mod_file == 'shops')
		{
			...
			// Cập nhật alias block shop
			$list_block = $db->query('SELECT bid, '. NV_LANG_DATA . '_alias as alias FROM ' . $db_config['prefix'] . '_' . $mod_data . '_block_cat')->fetchAll();
			foreach($list_block as $data){
				// Thêm id_alias chi tiết
				if(!in_array($data['alias'],$mod_alias)){
					$check_alias->add_block_shop_alias($data['bid'], $data['alias'], $mod_name, 'blockcat');
				}
			}
			...
			// XÓA ALIAS BLOCK SHOPS
			$alias_block_shops = $db->query('SELECT alias, id FROM ' . NV_PREFIXLANG . '_' . $module_data . '_rows WHERE module ="'. $mod_name .'" AND op="blockcat"')->fetchAll();
			
			$sql = 'SELECT '. NV_LANG_DATA . '_alias as alias FROM ' . $db_config['prefix'] . '_' . $mod_data . '_block_cat';
			$nv_Cache->delMod( $mod_name );
			$danhmuc = $nv_Cache->db($sql, 'alias', $mod_name);
		
			foreach($alias_block_shops as $data)
			{
				if (!isset($danhmuc[$data['alias']]))
				{
					// XÓA ALIAS 
		...
		// TRƯỜNG HỢP LÀ NEWS
		if($mod_file == 'news' || $mod_file == 'business' || $mod_file == 'tour')
		{
			// Cập nhật alias danh mục 
			//print(ass);die;
...
{
	//print_r($mod);die;
	if($mod['module_file'] == 'shops' or $mod['module_file'] == 'services' or $mod['module_file'] == 'page' or $mod['module_file'] == 'news' or $mod['module_file'] == 'business' or $mod['module_file'] == 'store' or $mod['module_file'] == 'download' )
	{
		$xtpl->assign( 'mod', $mod );
