...
if (!nv_function_exists('nv_block_data_config_banners_image')) {
    /**
     * nv_block_data_config_banners_image()
     *
     * @param mixed $module
     ...
     * @return
     */
    function nv_block_data_config_banners_image($module, $data_block, $lang_block)
    {
        ...
        $query = "SELECT * FROM " . NV_BANNERS_GLOBALTABLE . "_rows WHERE act = 1 ORDER BY weight ASC";
    ...
    /**
     * nv_block_data_config_banners_image_submit()
     *
     * @param mixed $module
     * @param mixed $lang_block
     * @return
     */
    function nv_block_data_config_banners_image_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    /**
     * nv_block_global_banners_image()
     *
     * @param mixed $block_config
     * @return
     */
    function nv_block_global_banners_image($block_config)
    {
        global $global_config, $client_info, $db;
        ...
        $array_banners_content = $db->query("SELECT * FROM " . NV_BANNERS_GLOBALTABLE . "_rows WHERE act = 1 AND id = ".$block_config['idplanbanner'])->fetchAll();
        ...
        if (!empty($array_banners_content)) {
            ...
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/banners/global.banners_image.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/banners/global.banners_image.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('global.banners_image.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/banners');

            foreach ($array_banners_content as $banners) {
                //var_dump($array_banners_content);die();
                $banners['file_image'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . NV_BANNER_DIR . '/' . $banners['file_name'];
                if (!empty($banners['bannerhtml'])) {
                    $banners['bannerhtml'] = html_entity_decode($banners['bannerhtml'], ENT_COMPAT | ENT_HTML401, strtoupper($global_config['site_charset']));
                }
...
if (defined('NV_SYSTEM')) {
    $content = nv_block_global_banners_image($block_config);
}
