...
class Location
{
    ...
    public function getArrayProvince($inArrayId = array(), $countryid = 0)
    {
        ...
        while ($row = $result->fetch()) {
            $row['name'] = ($module_config['location']['allow_type'] and ! empty($row['type'])) ? $row['title'] : $row['title'];
	 ...
	 public function buildInput($template = 'default', $module = 'location')
    {
	  ...
	  public function buildInput1($tinh,$quan,$template = 'default', $module = 'location')
    {
        global $site_mods, $global_config, $lang_module, $module_config;
        ...
        $xtpl = new XTemplate('form_input1.tpl', NV_ROOTDIR . '/themes/' . $template . '/modules/' . $site_mods[$module]['module_file']);
        $xtpl->assign('LANG', $lang_module);
        $xtpl->assign('CONFIG', 
            array(
                'select_countryid' => $this->select_countryid,
        ...
        if ($this->is_district) {
            if (! empty($array_district)) {
                ...
                foreach ($array_district as $districid => $district) {
                 // print_r($district['districtid']);die;
                    if($district['districtid'] == $quan)
	...
	public function buildInput2($template = 'default', $module = 'location')
    {
        global $site_mods, $global_config, $lang_module, $module_config;
