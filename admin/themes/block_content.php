...
if ($nv_Request->isset_request('confirm', 'post')) {
    ...
    $row['subtitle'] = nv_substr($nv_Request->get_title('subtitle', 'post', '', 1), 0, 255);
    ...
    if (! empty($error)) {
        ...
        if (is_array($array_funcid)) {
            ...
            if (empty($row['bid'])) {
                ...
                $_sql = "INSERT INTO " . NV_BLOCKS_TABLE . "_groups (theme, module, file_name, title, subtitle, link, template, position, exp_time, active, groups_view, all_func, weight, config) VALUES ( :selectthemes, :module, :file_name, :title, :subtitle, :link, :template, :position, '" . $row['exp_time'] . "', :active, :groups_view, '" . $row['all_func'] . "', '" . $row['weight'] . "', :config )";
                $data = array();
                ...
                $sth->bindParam(':subtitle', $row['subtitle'], PDO::PARAM_STR);
