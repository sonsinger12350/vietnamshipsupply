...
if( ! nv_function_exists( 'nv_message_page' ) )
{
	...
	function nv_message_page( $block_config )
	{
		...
		if( ! $is_show )
		{
			$sql = 'SELECT id,title,image,alias,description,bodytext,keywords,add_time,edit_time FROM ' . NV_PREFIXLANG . '_' . $site_mods[$module]['module_data'] . ' WHERE status=1 ORDER BY rand() DESC';
			...
			if( ( $query = $db->query( $sql ) ) !== false )
			{
				if( ( $row = $query->fetch() ) !== false )
				{
					...
					$image = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module . '/'. $row['image'];
					$title = $row['title'];
					$bodytext = $row['description'];
					$bodytext = nv_clean60( $bodytext, 800 );
