...
if( ! nv_function_exists( 'nv_menu_site_mods' ) )
{
	...
	function nv_menu_site_mods( $block_config )
	{
		...
		foreach( $site_mods as $modname => $modvalues )
		{
			...
			{
				if( $modvalues['module_data'] == 'supplies')
				$link = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $modname ."&op=supplies";
				else $link = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $modname ;
				$array_menu = array(
