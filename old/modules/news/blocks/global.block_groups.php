...
if( ! nv_function_exists( 'nv_block_news_groups' ) )
{
	...
	function nv_block_news_groups( $block_config )
	{
		...
		$db->sqlreset()
			->select( 't1.id, t1.catid, t1.admin_id, t1.title, t1.alias, t1.homeimgfile, t1.homeimgthumb,t1.hometext,t1.publtime' )
		...
		if( ! empty( $list ) )
		{
			if( file_exists( NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/news/block_groups1.tpl' ) )
			{
			...
			$xtpl = new XTemplate( 'block_groups1.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/news' );
			$i = 1;
			foreach( $list as $l )
			{
				$l['publtime'] = nv_date( 'l - d/m/Y H:i', $l['publtime'] );
				...
				$l['title'] = nv_clean60( $l['title'], 60, true );

				if( ! $block_config['showtooltip'] )
				{
					$xtpl->assign( 'TITLE', 'title="' . $l['title'] . '"' );
				}
				$sql = "select username from nv4_users where userid = ". $l['admin_id'];
				$kq = $db->query($sql);
				$rowne = $kq->fetch();
				$l['username'] = $rowne['username'];
				if($i == 1)
				{
					$xtpl->assign( 'ROW1', $l );
				}
