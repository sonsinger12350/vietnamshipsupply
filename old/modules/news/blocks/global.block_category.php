...
if( !nv_function_exists( 'nv_news_category' ) )
{
	...
	{
		global $module_array_cat, $module_info, $lang_module, $global_config, $catid, $id;
		...
		if( !empty( $module_array_cat ) )
		{
			...
			foreach( $module_array_cat as $cat )
			{
				if( $block_config['catid'] == 0 && $cat['parentid'] == 0 || ($block_config['catid'] > 0 && $cat['parentid'] == $block_config['catid']) )
				{
					...
					if( !empty( $cat['catid'] ) )
					{
						$html .= nv_news_sub_category( $cat['catid'], $title_length );
					}
				}
			}
	...
	function nv_news_sub_category( $catid_dm, $title_length )
	{
		global $module_data,$global_array_cat, $module_name, $db ,$global_config, $site_mod , $id, $module_array_cat;
		
		$result = $db->query( 'SELECT id,catid,title,alias FROM ' . NV_PREFIXLANG . '_supplies_rows WHERE status=1 AND catid = ' . $catid_dm );
		...
		if( $rating_count > 0 )
		{
			...
			while( $row = $result->fetch( ) )
			{
				...
				$link = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=supplies&amp;' . NV_OP_VARIABLE . '=' . $module_array_cat[$catid_dm]['alias'] . '/' . $row['alias'] . '-' . $row['id'] . $global_config['rewrite_exturl'];
...
if( defined( 'NV_SYSTEM' ) )
{
	...
	if( isset( $site_mods[$module] ) )
	{
		...
		else
		{
			...
			$sql = "SELECT catid, parentid, title, alias, viewcat, subcatid, numlinks, description, inhome, keywords, groups_view FROM " . NV_PREFIXLANG . "_supplies_cat ORDER BY sort ASC";
