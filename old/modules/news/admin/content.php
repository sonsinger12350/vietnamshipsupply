	...
	if( empty( $error ) )
	{
		...
		if( $rowcontent['id'] == 0 )
		{
			...
			$sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_rows
				(catid, listcatid, topicid, admin_id, author, sourceid, addtime, edittime, status, publtime, exptime, archive, title, alias, hometext, homeimgfile, homeimgalt, homeimgthumb,otherimage, inhome, allowed_comm, allowed_rating, hitstotal, hitscm, total_rating, click_rating) VALUES
...
if( !empty( $otherimage ) )
{
	foreach( $otherimage as $otherimage_i )
	{
		...
		$xtpl->assign( 'CURRENT1', NV_UPLOADS_DIR . '/' . $module_upload . '/' . date( 'Y_m' ) );
